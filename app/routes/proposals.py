...
from app.models import Proposal, ProposalItem, Client, Service, ServicePackage
...
def create(client_id=None):
    ...
    with get_db() as db:
        ...
        # Get all service packages for selection
...
def edit(proposal_id):
    ...
    with get_db() as db:
        ...
        return render_template('proposals/edit.html', proposal=proposal, services=services, packages=packages)
...
def accept(proposal_id):
    """Aceitar proposta e iniciar workflow (Financeiro + Produção)"""
    with get_db() as db:
        proposal = db.query(Proposal).filter(Proposal.id == proposal_id).first()
        ...
        # 2. Mover o Deal para "Ganho"
        from app.models import Deal, KanbanStage
        deal = db.query(Deal).filter(Deal.client_id == proposal.client_id, Deal.status == 'open').first()
        
        if deal:
            # Encontrar etapa "Fechado - Ganho"
            ganho_stage = db.query(KanbanStage).filter(KanbanStage.name.like('%Ganho%')).first()
            if ganho_stage:
                old_stage_name = deal.kanban_stage.name if deal.kanban_stage else None
                deal.kanban_stage_id = ganho_stage.id
                
                # Sincronizar campo Stage do cliente (se existir)
                if proposal.client:
                    proposal.client.kanban_stage_id = ganho_stage.id
                
                # 3. Trigger Workflow via Service
                from app.services.workflow import WorkflowService
                WorkflowService.on_deal_stage_changed(db, session.get('company_id'), deal, old_stage_name, ganho_stage.name)
        
        db.commit()
        flash('Proposta aceita! O projeto e as faturas foram gerados automaticamente.', 'success')
        return jsonify({'success': True, 'redirect': url_for('proposals.view', proposal_id=proposal.id)})
